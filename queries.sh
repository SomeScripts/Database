...
echo "$($PSQL "SELECT name FROM teams WHERE team_id IN (SELECT UNNEST(array[winner_id, opponent_id]) FROM games WHERE round='Eighth-Final' AND year='2014') ORDER BY name")"
